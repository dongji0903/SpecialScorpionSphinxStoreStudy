...
<!--=====================
          Content
======================-->
<form id="frm" action="<%=request.getContextPath()%>/admin/product/edit" method="post"  enctype="multipart/form-data">
<input type="hidden" name="productId" value="${product.productId}"/>
<input type="hidden" name="originCategoryId" value="${category.categoryId}"/>
<section id="imageSection" class="content">
            ...
            <div class="grid_5" id="conten">
              <h2>대표 이미지</h2>
              	...
              	<img alt="" src="" style="display:none; max-width: 30%;" id="preview"><br><br>
				    ...
				    <input type="text" style="padding-left: 10px;" readonly="readonly" title="File Route" id="1000text" value="${product.fileName.get(0)}">
					<c:if test="${product.fileName.size() > 1}">
						<c:forEach begin="1" end="${product.fileName.size() - 1}" var="pimg" items="${product.fileName}">
							<div class='file_input' style='margin-top:15px;'>
							<label for='"+result+"'>File Upload
							<input type='file' name='fileNm' id='"+result+"' class='1000' onchange='change("+ result +", this.value);'>
							</label><label style='border-left: solid 0.5px white;' class='remove'>삭제</label><label id='"+result+"p' onclick='preview(this.id)' style='border-left: solid 0.5px white; width: 25%;' id='upplus'>미리보기</label>
							<input type='text' value="${pimg}" style='padding-left: 10px;' readonly='readonly' id='"+ result +"text'/></div>
						</c:forEach>
					</c:if>
				</div>
				</div>
...
<section id="informationSection" class="content">
              ...
              <h2>상품 속성</h2>
              	...
              	<select name="categoryId" style="height: 25px; width: 80%; overflow-x: auto;">
              		...
              		<c:forEach var="sub" items="${categoryList}">
	              		<c:if test="${parents.get(sub.categoryId).size() >= 2 && sub.categoryId != category.categoryId}">
		              		<option value="${sub.categoryId}">
              				<c:forEach var="parent" items="${parents.get(sub.categoryId)}">
              					${parent.categoryName} >
              				</c:forEach>
              					${sub.categoryName}
						</option>
              	...
              	상품명 : 
              	<input type="text" id="name" value="${product.productName}" name="productName" placeholder="상품명을 입력해주세요.";
              	style="width: 80%; height: 20px; padding-left:5px; padding-right:5px; font-size: 13px;"><br>
              	가&nbsp;&nbsp;&nbsp;&nbsp;격 : 
              	<input type="text" id="price" value="${product.price}" name="price" placeholder="상품의 가격을 입력해주세요.";
              	style="width: 80%; height: 20px; padding-left:5px; padding-right:5px; font-size: 13px;"><br>
              	코멘트 : 
              	<input type="text" id="comment" value="${product.comment}" name="comment" placeholder="해당 상품만의 특별한 코멘트를 입력해주세요.";
              	style="width: 80%; height: 20px; padding-left:5px; padding-right:5px; font-size: 13px;"><br>
            </div>
          </div> 
        </div>
...
<section id="optionSection" class="content">
...
<section id="contentSection" class="content">
  <div class="container" id="page4" style="display:none;">
    <div class="row">
          ...
          <div class="row">
            <div class="grid_5 preffix_1" style="width: 80%;">
              ...
              <textarea id="ir1" cols="10" onkeydown="limit(this.form)"
						name="content" class="form-control" style="width: 100%; height: 300px;">
			  </textarea>
            </div>
          </div> 
        </div>
      ...
      <div align="center">
      <span id="warnn" style="color: red;"></span><br>
      	<a class="btn" id="sub" onclick="moveBefore(4)" style="cursor: pointer;">이전</a> <a class="btn" id="save" style="cursor: pointer;">수정 완료</a>
      </div>
    </div>
  </div>
</section>
